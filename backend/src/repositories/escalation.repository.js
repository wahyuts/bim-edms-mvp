...
async function listOverdue(filters, pagination) {
  const params = {};
  ...
  const condition = `d.deleted_at IS NULL
    ...
    AND (
      d.sla_status = 'Overdue'
      OR (d.sla_due_at IS NOT NULL AND d.sla_due_at < UTC_TIMESTAMP())
    )
    ${searchSql}`;
...
async function findOverdueById(id) {
  const [rows] = await pool.execute(
    `SELECT d.*, r.name AS repository_name
    FROM documents d
    LEFT JOIN repositories r ON r.id = d.repository_id
